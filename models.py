...
from losses2 import CustomRoIHeads
...
def model_selector(train_s, label2id, device, model_type, pretrained, num_classes):
    ...
    if model_type=='base':
        ...
        class_weights = torch.clamp(class_weights, max=3.0) #[2-0, 5.0]

        model = build_model(
            num_classes=num_classes,
            class_weights=class_weights,
            dice_alpha=0.3,          # mix between (BCE/Focal) and Dice
            pretrained=pretrained,
            use_bce=True,            # enable BCE
            use_focal=False,         # also add Focal (optional)
            focal_alpha=0.25,
            focal_gamma=2.0,
        ).to(device)

...
from balancing import injury_label_remap, downsample_to_min_per_class
...
def prepare_structured_dataset_df(
    ...
    df = df[df['outcome_label']!='healthy']
    #df = downsample_to_min_per_class(df, label_field)
    ...
    for _, row in df.iterrows():
        ...
        out.append({
            ...
            "case_label": row.get(label_field, "unknown"),
            "regions": regs
        })
    return out
...
def create_hist(train_split: List[Dict[str, Any]],
    ...
    ax = counts.plot(kind="barh", stacked=True, figsize=(15, 15), colormap="tab20")
